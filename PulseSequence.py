...
class PulseSequence:
    ...
    """
    def const_pulse(self, wd, amp, t_pulse, t_start=None, t_rise=1):
        if t_start is None: t_start = self.time
        def envelope(t):
                t -= t_start 
    ...
    Returns the total length of the sequence.
    """
    def gaussian_pulse(self, wd, amp, t_pulse_sigma, t_start=None):
        if t_start is None: t_start = self.time
        def envelope(t):
